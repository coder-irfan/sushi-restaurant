...
import { FaPlusCircle, FaUtensils, FaRegCalendarCheck, FaUsers, FaArrowRight, FaHistory } from "react-icons/fa";
...
function DashboardHome() {
  ...
  const cards = [
    ...
    { title: "Reservations", icon: <FaRegCalendarCheck />, link: "reservations" },
    ...
    <>
      <section className="2xl:max-w-[100rem] mx-auto px-4 py-10 sm:px-6 md:px-8 md:py-4 lg:p-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 place-items-center gap-y-8 sm:gap-y-4 sm:gap-x-6 justify-between">
          {cards.map((card, idx) => (
            <div 
              className={`${card.color} flex min-h-48 lg:min-h-60 bg-deepGray border-l-[0.4rem] border-l-goldYellow w-full p-4 md:p-6 shadow-md shadow-darkCharcoal
              ...
              <div className="flex flex-col justify-between">
                <div className="space-y-4">
                  ...
                  <p className="text-lightGray text-sm lg:text-base">
                    {
                      card.title === "Manage Sushis" ? `Total sushis: ${stats.sushis}`:
                      card.title === "Orders" ? `Total orders: ${stats.orders}`:
                      card.title === "Reservations" ? `Total reservations: ${stats.reservations}` :
                      card.title === "Users" ? `Total registerations: ${stats.users}` : 
                      card.title === "Activity" ? `Total activities: ${stats.activity}` : 
                      ""
                    }
                  </p>
                </div>
                ...
                <div className="">
                  <Link
                    to={card.link}
                  >
                    <button className="flex items-center gap-4 border border-goldYellow px-4 py-1.5 rounded-full group relative hover:text-goldYellow
